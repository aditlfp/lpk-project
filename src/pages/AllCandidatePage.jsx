...
const renderStars = (rating) => {
  ...
  if (hasHalf) {
    stars.push(
      <StarHalf key="half" size={16} className="text-yellow-400 fill-current" />
    );
  }
  return stars;
};
...
const mapStudent = (item, base) => {
  if (!item.best_student) return null;
  return {
    ...
    imageUrl: item.best_student?.document?.file_path
      ? `${base}/${item.best_student.document.file_path}`
      : `https://placehold.co/100x100/EAD9F2/333?text=${item.best_student?.nama_lengkap?.charAt(
          0
        )}`,
  };
};

const mapOfficer = (item, base) => {
  if (!item.field_officiers) return null;
  return {
    ...
    imageUrl: item.field_officiers?.document?.file_path
      ? `${base}/${item.field_officiers.document.file_path}`
      : `https://placehold.co/100x100/D4E7C5/333?text=${item.field_officiers?.nama_lengkap?.charAt(
          0
        )}`,
  };
};

const mapSensei = (item, base) => {
  if (!item.sensei) return null;
  return {
    id: item.sensei?.id,
    ...
    imageUrl: item.sensei?.document?.file_path
      ? `${base}/${item.sensei.document.file_path}`
      : `https://placehold.co/100x100/FFD8B1/333?text=${item.sensei?.nama_lengkap?.charAt(
          0
        )}`,
  };
};
...
function AllCandidatePage({ onBackClick }) {
  ...
  const fetchData = async (url) => {
    ...
    try {
      ...
      setStudents(
        candidatesData.map((i) => mapStudent(i, storageBaseUrl)).filter(Boolean)
      );

      setOfficers(
        candidatesData.map((i) => mapOfficer(i, storageBaseUrl)).filter(Boolean)
      );

      setSensei(
        candidatesData.map((i) => mapSensei(i, storageBaseUrl)).filter(Boolean)
      );
  ...
  const renderContent = () => {
    if (isLoading) {
      return (
        <p className="text-center text-gray-500 col-span-full">Memuat...</p>
      );
    }
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8 max-w-lg mx-auto">
          <input
            ...
            className="w-full px-5 py-3 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-shadow"
            value={searchTerm}
        ...
        <div className="mt-12 flex justify-between items-center gap-x-4 md:gap-x-0">
          <button
            ...
            className="px-5 py-2.5 bg-indigo-600 text-white font-semibold rounded-lg shadow-md hover:bg-indigo-700 disabled:bg-gray-300 disabled:cursor-not-allowed transition-all duration-300"
          >
            Sebelumnya
          </button>
          ...
          <button
            ...
            disabled={!pagination.nextUrl || isLoading}
            className="px-5 py-2.5 bg-indigo-600 text-white font-semibold rounded-lg shadow-md hover:bg-indigo-700 disabled:bg-gray-300 disabled:cursor-not-allowed transition-all duration-300"
          >
            Berikutnya
          </button>

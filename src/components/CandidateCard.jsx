  ...
  apiData.forEach((period) => {
    ...
    if (period.best_student) {
      formattedData.push({
        ...
        image: `https://placehold.co/600x400/003366/FFFFFF?text=${getInitials(
          period.best_student.nama_lengkap
        )}`,
        description:
          period.best_student.prestasi ||
          "Siswa dengan prestasi gemilang selama masa pendidikan.",
        details: {
    ...
    if (period.sensei) {
      formattedData.push({
        ...
        image: `https://placehold.co/600x400/EFEFEF/000000?text=${getInitials(
          period.sensei.nama_lengkap
        )}`,
        description: `Keunggulan: "${period.sensei.keunggulan_diri}"`,
        details: {
          "Nama Lengkap": period.sensei.nama_lengkap,
          "Bidang Keahlian":
            period.sensei.bidang_keahlian || "Tidak disebutkan",
...
function CandidateCard({ onSeeAllClick }) {
  ...
  const currentIndex =
    awardWinnersData.length > 0
      ? page < 0
        ? (awardWinnersData.length -
            (Math.abs(page) % awardWinnersData.length)) %
          awardWinnersData.length
      ...
      <div className="bg-[#F4F8FB] min-h-screen font-sans flex flex-col items-center justify-center p-4 sm:p-6 lg:p-8">
        <div className="w-full max-w-6xl mx-auto">
          ...
          <div className="relative h-[550px] sm:h-[480px] md:h-[400px]">
            <AnimatePresence initial={false} custom={direction}>
              ...
              >
                <div className="bg-white shadow-lg rounded-2xl overflow-hidden md:flex h-full">
                  <div className="md:w-1/2 flex-shrink-0 h-80 md:h-auto bg-gray-100 flex items-center justify-center rounded-l-2xl overflow-hidden">
                    <img
                      src={
                        person?.document?.file_path != undefined
                          ? import.meta.env.VITE_STORAGE_IMG +
                            person?.document?.file_path
                          : person.image
                      }
                  ...
                  <motion.div
                    className="p-0 px-6 md:p-8 lg:p-12 flex flex-col justify-center md:w-1/2"
                  ...
                  >
                    <motion.h3
                      ...
                      className="text-2xl lg:text-3xl font-bold text-[#003366]"
                    >
                      {person.category}
                    </motion.h3>
                    <motion.p
                      variants={contentItemVariants}
                      className="text-lg font-medium text-gray-800 mt-1"
                    >
                      {person.name}
                    </motion.p>
                    <motion.p
                      variants={contentItemVariants}
                      className="mt-4 text-gray-600 flex-grow"
                    >
                      {person.description}
                    </motion.p>
                    <motion.div variants={contentItemVariants}>
                      <motion.button
                        ...
                        whileTap={{ scale: 0.95 }}
                        className="my-6 w-full sm:w-auto self-start bg-[#F5A200] text-white font-bold py-3 px-8 rounded-full hover:bg-[#E09300] focus:outline-none focus:ring-4 focus:ring-[#F5A200]/50"
                      >
                        Lihat Detail
                      </motion.button>
                    </motion.div>
            ...
            <button
              onClick={() => paginate(-1)}
              className="absolute top-1/2 left-2 md:-left-4 transform -translate-y-1/2 bg-white/70 hover:bg-white rounded-full p-2 shadow-md transition z-10"
            >
              <ChevronLeft className="text-[#003366]" size={28} />
            </button>
            <button
              onClick={() => paginate(1)}
              className="absolute top-1/2 right-2 md:-right-4 transform -translate-y-1/2 bg-white/70 hover:bg-white rounded-full p-2 shadow-md transition z-10"
            >
        ...
        <AnimatePresence>
          {isModalOpen && (
            <DetailModal person={selectedPerson} onClose={handleCloseModal} />
          )}
        </AnimatePresence>
      </div>
    </>
